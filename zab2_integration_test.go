...
func TestThresholds(t *testing.T) {
	for _,apiId := range updatetests {
		...
		if (!strings.Contains(string(body), fmt.Sprintf("threshold\":{ \"value\":%s, \"lower\":%s }",apiId.oldthreshold,apiId.oldlower))) {
			t.Errorf("%s","wrong threshold or lower")
		}
	}
}
...
func TestUpdateThreshold(t *testing.T) {
	...
	//	_, err := http.PostForm("http://localhost:8080/api/1", form)	
	...
	//	}
		...
		if (!strings.Contains(string(body), fmt.Sprintf("threshold\":{ \"value\":%s, \"lower\":%s }",apiId.oldthreshold,apiId.oldlower))) {
			t.Errorf("%s","update failed, wrong threshold or lower")
		}
	}
}
...
func init() {
	...
	server = exec.Command("go","run","zab2.go","-s","multi-axis-test")
	err = server.Start()
